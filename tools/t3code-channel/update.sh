...
write_health() {
  ...
  node -e '
    ...
    const [output, status, incidentKey, summary, stage, originSha, mainSha, prSha, integrationSha, workflowUrl] = process.argv.slice(1);
  ...
  ' "$temporary_health" "$status" "$incident_key" "$summary" "$current_stage" \
    "$origin_sha" "$main_sha" "$pr_sha" "$integration_sha" "$workflow_url"
...
record_unexpected_failure() {
  ...
  write_health \
    "failed" \
    "unexpected:${current_stage}:${origin_sha:-unknown}:${main_sha:-unknown}:${pr_sha:-unknown}" \
