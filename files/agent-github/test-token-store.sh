...
SOPS_SECRET="$TEST_ROOT/run/secrets.d/1/agent-github-token"
SOPS_LINK="$TEST_ROOT/run/secrets/agent-github-token"
mkdir -p "$(dirname "$SOPS_SECRET")" "$(dirname "$SOPS_LINK")"
printf '%s' "$FIXTURE_TOKEN" > "$SOPS_SECRET"
chmod 400 "$SOPS_SECRET"
ln -s "$SOPS_SECRET" "$SOPS_LINK"
sed -i "s|^AGENT_GITHUB_TOKEN_FILE=.*|AGENT_GITHUB_TOKEN_FILE=$SOPS_LINK|" \
  "$XDG_CONFIG_HOME/codex-github-machine/setup.env"

if [[ "$("$TOKEN_STORE" lookup)" != "$FIXTURE_TOKEN" ]]; then
  printf 'sops-nix-style token symlink was not readable\n' >&2
  exit 1
fi
