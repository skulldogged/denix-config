...
MANAGED_REGISTRY="$TEST_ROOT/managed/repositories"
mkdir -p "$(dirname "$MANAGED_REGISTRY")"
printf '%s\n' 'example/allowed' > "$MANAGED_REGISTRY"
chmod 444 "$MANAGED_REGISTRY"
chmod 555 "$(dirname "$MANAGED_REGISTRY")"
if MANAGED_OUTPUT=$(AGENT_GITHUB_REPOSITORIES_FILE="$MANAGED_REGISTRY" \
  "$AGENT_GH" register example/second 2>&1); then
  printf 'expected declaratively managed registry update to fail\n' >&2
  exit 1
fi
assert_contains "$MANAGED_OUTPUT" 'repository registry is managed declaratively'
assert_contains "$MANAGED_OUTPUT" 'programs.agent-github.repositories'
